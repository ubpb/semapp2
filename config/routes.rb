...
ActionController::Routing::Routes.draw do |map|
  ...
  map.namespace :admin do |admin|
    ...
    admin.resources :books, :only => [:edit, :update, :destroy, :defer, :dedefer, :placed_in_shelf, :removed_from_shelf], :member => {:defer => :put, :dedefer => :put, :placed_in_shelf => :put, :removed_from_shelf => :put}
  ...
  # Sem Apps
  map.resources :sem_apps, :as => 'apps', :controller => 'sem_apps', :member => {:unlock => :post, :transit => :post, :clones => :get, :clone => :post, :filter_clones => :post, :clear => :delete}, :collection => {:filter => :post} do |sem_app|
